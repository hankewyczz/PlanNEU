...
class Result {
  ...
  areDaysFree: { [key: string]: boolean } = {
    "1": true,
    "2": true,
    "3": true,
    "4": true,
  ...
  constructor(sections: Section[]) {
    ...
    for (let sec of sections) {
      ...
      if (sec.times != null) {
        this.earliestStart = Math.min(
          sec.times.earliestStart,
          this.earliestStart
        );
        this.latestEnd = Math.max(sec.times.latestEnd, this.latestEnd);
      }

      this.minSeatsLeft = Math.min(
        sec.content["seatsRemaining"],
        this.minSeatsLeft
      );
    ...
    this.daysOff = Object.keys(this.areDaysFree).filter(
      (day) => this.areDaysFree[day]
    ).length;
  }
  ...
  timeStr(time: Time, section: Section): string {
    return `${secsToHM(time.start)} - ${secsToHM(time.end)} | ${
      section.fullName
    }`;
  }
