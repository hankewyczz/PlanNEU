...
class Course {
  ...
  sections(): Section[] {
    ...
    return content.sections.map(
      (sec: SectionContent) => new Section(this, sec)
    );
  }
}
...
function getQueryUrl(course: Course): string {
  ...
  queryURL += addQuery(
    "classIdRange",
    `min-${course.courseId}_max-${course.courseId}`
  );
...
function getCoreqs(course: Course): string {
  ...
  function coreqCase(values: any[], type: string): string | null {
    ...
    const outputArr: string[] = mappedVals.filter(
  ...
  return coStr === null
    ? ""
    : `Corequisite courses (click to add): ${coStr.trim()}`;
}
