...
function preferredProfs(result: Result, profs: string[]): boolean {
  for (let professor of profs) {
    let index = result.professors.findIndex(
      (item) => professor.toLowerCase() === item.toLowerCase()
    );
...
function isValidTime(
  sec: Section,
  start: number = MIN_TIME,
  end: number = MAX_TIME
): boolean {
  return (
    sec.times === null ||
    (sec.times.earliestStart >= start && sec.times.latestEnd <= end)
  );
}
