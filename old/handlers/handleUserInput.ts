...
async function handleSingleCourse(
  input: string = (document.getElementById(USER_INPUT_ID) as HTMLInputElement)
    .value,
  semester: string = (
    document.getElementById(SEMESTER_SELECTOR_ID) as HTMLInputElement
  ).value
): Promise<void> {
  try {
    ...
    let coursesAdded =
      (document.getElementById(COURSES_DIV_ID) as HTMLElement).children.length -
      COURSE_DIV_KIDS;
    ...
    handleMessage(
      `Successfully added ${course.fullName}! ${getCoreqs(course)}`,
      Message.Success
    );
  } catch (err) {
    console.log(err);
...
function addToCourseDiv(course: Course, fullCourseName: string): void {
  ...
  remove.onclick = function () {
    handleRemove(course.name, courseEntry);
  };
...
function handleMessage(message: string, type: Message = Message.Basic): void {
  console.log(message);
  let messageDiv: HTMLElement = document.getElementById(
    MESSAGE_DIV_ID
  ) as HTMLElement;
...
function toggleInstructions(): void {
  let instructions: HTMLElement = document.getElementById(
    "instructions"
  ) as HTMLElement;
...
function setValues() {
  ...
  (document.getElementById("submit-input-courses") as HTMLInputElement).value =
    courses.join();
}
